...
CONST  حظة  =  تتطلب ( "لحظة" ) 
CONST  ytdl  =  تتطلب ( "ytdl النواة" )؛ 
CONST  {  العميل ،  UTIL  }  =  تتطلب ( 'discord.js' )؛ 
CONST  getYoutubeID  =  تتطلب ( 'الحصول يوتيوب معرف " )؛ 
...
client . على ( 'ready' ،  ()  =>  { // 
    وحدة msgs جاهزة . log ( `Logged in as $ { client . user . tag } !` )؛ 
    console . log ( `in $ { client . guilds . size } خوادم ` ) 
    وحدة التحكم . سجل ( ` [] $ { العميل . المستخدمين . حجم} ` ) 
}) ؛ 

العميل . على ( 'message' ،  async  msg  =>  { 
	if  ( msg . author . bot )  return  undefined ؛ 
	if  ( ! msg . content . beginWith ( prefix ))  return  undefined ؛ 
	const  args  =  msg . content . split ( '' )؛ 
	const  searchString =  args . شريحة ( 1 ). انضم ( ' ) ؛ 
	CONST  رابط  =  وسائط [ 1 ]  ؟  args [ 1 ]  . استبدال ( /<(.+)>/g ،  '$ 1' )  :  '' ؛ 
	const  serverQueue  =  قائمة الانتظار . get ( msg . guild . id )؛ 
	اترك  الأمر  =  msg . المحتوى . toLowerCase(). split ( "" ) [ 0 ]؛ 
	الأمر  =  الأمر . شريحة ( البادئة . الطول ) 
	إذا كان  ( command  ===  `play` )  { 
		const  voiceChannel  =  msg . عضو . voiceChannel . 
		إذا  ( ! VoiceChannel ) قم  بإرجاع  msg . قناة . أرسل ( 'يجب توآجد حضرتك بروم صوتي.' )؛ 
		أذونات const  =  قناة الصوت . permissionsFor ( msg . client . user )؛ 
		إذا  ( ! الأذونات . و ( 'الاتصال' ))  { 
			عودة  جي اس . قناة . أرسل ( 'لا يتوآجد لدي صلاحية للتكلم بهذآ الروم' )؛ 
		} 
		إذا كانت  ( ! الأذونات . لديها ( 'SPEAK' ))  { 
			return  msg . قناة . أرسل ( 'لا يتوآجد لدي صلاحية للتكلم بهذآ الروم')؛ 
		} 

		إذا كانت  ( ! الأذونات . لديها ( 'EMBED_LINKS' ))  { 
			return  msg . قناة . sendMessage ( "** يجب توآفر برمشن` EMBED LINKS` لدي ** rl " ) 
			} 

		إذا كان  ( url . match ( / ^ https؟: \ / \ / (www.youtube.com | youtube.com) \ / playlist (. *) $ / ))  { 
			const  playlist  =  await  youtube . getPlaylist ( رابط )؛ 
